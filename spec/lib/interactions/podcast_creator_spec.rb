...
describe Interactions::PodcastCreator do
  let(:account) {
    PodcastSyncerModel::Account.new
  }
  let(:podcast_creator) {
  ...
  describe "#execute" do
    it 'create the podcast with params' do
      ...
      end.should change(PodcastSyncerModel::Podcast, :count)
    end
  end

  describe "#persisted?" do
    it 'true if podcast is save' do
      PodcastSyncerModel::Podcast.expects(:new).returns(mock(:save => true))
      podcast_creator.execute
      podcast_creator.persisted?.should be_true
    end

    it 'false if podcast is save' do
      PodcastSyncerModel::Podcast.expects(:new).returns(mock(:save => false))
      podcast_creator.execute
